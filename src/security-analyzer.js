...
class SecurityAnalyzer {
   ...
   * Pre-analysis security checks
   */
  async performPreAnalysis (code, _context) {
    const findings = [];
   ...
   * Pattern-based vulnerability detection
   */
  async performPatternAnalysis (code, _context) {
   ...
   * Dependency security analysis
   */
  async analyzeDependencies (code, _context) {
    const dependencies = [];
    const vulnerabilities = [];
   ...
   */
  combineAnalysisResults (res) {
    const allVulnerabilities = [];
    const allWarnings = [];

    // Combine pre-analysis findings
    if (res.preAnalysis.findings) {
      allVulnerabilities.push(...res.preAnalysis.findings);
    }

    // Combine AI analysis findings
    if (res.aiAnalysis.success && res.aiAnalysis.analysis.vulnerabilities) {
      allVulnerabilities.push(...res.aiAnalysis.analysis.vulnerabilities);
    }

    if (res.aiAnalysis.success && res.aiAnalysis.analysis.warnings) {
      allWarnings.push(...res.aiAnalysis.analysis.warnings);
    }

    // Combine pattern analysis findings
    if (res.patternAnalysis.vulnerabilities) {
      allVulnerabilities.push(...res.patternAnalysis.vulnerabilities);
    }

    // Combine dependency analysis findings
    if (res.dependencyAnalysis.vulnerabilities) {
      allVulnerabilities.push(...res.dependencyAnalysis.vulnerabilities);
    }
...
5. Best practices advice`;
  ...
  async checkVulnerabilityDatabase (_packageName, _version) {
